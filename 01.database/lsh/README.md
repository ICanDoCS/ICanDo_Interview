...
트랜잭션을 자주 커밋합니다. 정해진 순서로 테이블에 접근합니다. 읽기 잠금 획득 (SELECT ~ FOR UPDATE)의 사용을 피합니다. 한 테이블의 복수 행을 복수의 연결에서 순서 없이 갱신하면 교착 상태가 발생하기 쉬운데 이 경우에는 테이블 단위로 잠금을 걸어두고 갱신을 직렬화하면 동시성은 떨어지지만 교착상태를 회피할 수 있습니다.

예방 기법, 회피 기법, 회복 기법, 발견 기법
